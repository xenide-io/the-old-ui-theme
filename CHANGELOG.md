...
- Unread badge sits on the notification button corner so it no longer covers the bell.
...
- Unread count sits on the notification bell icon instead of the far corner of the hit box.
...
- Shelly's empty state now waits for a question instead of offering to dump the workspace.
...
- Kraken light canvas now lives in a single `[data-theme="kraken-light"]` token instead of per-app colour overrides.

### Fixed

- Theme colour meta now syncs to the initial theme on first mount instead of only when the theme toggles.

## [0.6.6] - 2026-08-18

### Changed

- Kraken suite landing is now Deep Research (`/ask`).
